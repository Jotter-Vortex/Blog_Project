...
@app.route('/memomain')                                 # home 주소
def getmemo_main():
   return render_template('memo_main.html')

@app.route('/blog_about')                                 # about 주소
def about():
   return render_template('portfolio.html')

@app.route('/blog_post')                                 # post 주소
def posting():
   return render_template('post.html')

@app.route('/blog_contact')                                 # contact 주소
def contact():
   return render_template('contact.html')

@app.route('/dream')                                 # contact 주소
def dream():
...
def login():
    ...
    pw = request.form['PASSWORD'] # 클라이언트로부터 content를 받을 부분
    # 2. mongoDB에 데이터 찾기
    tclist = list(db.user_info.find({'uid': id}))
    if id ==tclist['uid'] and pw == tclist['upw']:
        loginstate = '1'
    else:
        loginstate = '0'
    return jsonify({'result': 'success', 'loginstate' : loginstate})
    #, 'msg':'POST 연결되었습니다!'})
...
def show_notepad():
    ...
    # 2. mongoDB에 데이터 넣기'
    nowtime = str(datetime.datetime.now())           # 현재 시간을 가져옵니다. 최초 메모를 쓸때 사용됩니다.
...
def post_notepad():
    ...
    nowtime = str(datetime.datetime.now())           # 현재 시간을 가져옵니다. 메모를 수정하면 시간이 바뀝니다.
    nowtime = nowtime.split('.')
    # 2. DB에 데이터를 찾기
    ...
    db.notepad.update_one({'_id': ObjectId(index)}, {'$set': {'time': nowtime[0]}})
    return jsonify({'result': 'success'})
    #, 'msg':'POST 연결되었습니다!'})
